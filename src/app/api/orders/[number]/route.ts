...
export async function GET(request:Request,{params}:{params:Promise<{number:string}>}){
  ...
  const orders=await db()<OrderRow[]>`SELECT id,number,customer_id,status,payment_status,fulfillment_status,subtotal,decoration_total,discount_total,shipping_total,grand_total,discount_code,shipping_address,tracking_number,courier_name,shipment_status,tracking_url,created_at FROM orders WHERE number=${number} AND (customer_id=${user?.id??null} OR (${tokenHash}::text IS NOT NULL AND access_token_hash=${tokenHash})) LIMIT 1`;
  ...
  return NextResponse.json({order:{
    ...
    coupon:order.discount_code,address:order.shipping_address,trackingNumber:order.tracking_number,courierName:order.courier_name,shipmentStatus:order.shipment_status,trackingUrl:order.tracking_url,
