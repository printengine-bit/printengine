...
type ProductRow={id:string;slug:string;name:string;subtitle:string;category:Product["category"];kind:Product["kind"];fit:string;gsm:number;base_price:number;compare_at_price:number;audience:Audience[];methods:Method[];media:unknown;featured:boolean;is_new:boolean};
...
export async function commerceProducts():Promise<Product[]>{
  ...
  try{
    const [productRows,variantRows]=await Promise.all([
      db()<ProductRow[]>`SELECT id,slug,name,subtitle,category,kind,fit,gsm,base_price,compare_at_price,audience,methods,media,featured,is_new FROM products WHERE status='active' ORDER BY created_at`,
