...
6. Remove `ADMIN_PASSWORD` from the runtime environment after seeding.
...
an order. Browser totals are estimates only. Payment callbacks and webhooks are verified with HMAC,
and paid-order processing is idempotent: inventory and discount usage are deducted exactly once.
...
- Add email verification, password reset and login rate limiting.
- Add database-backed rate limits for AI generation, uploads and authentication.
- Sanitize SVG uploads and run malware scanning before production access.
